...
console.log(midSumMas([1,2,3]));
// конец  среднее арифметическое в массиве
// 
let i = 5;
let f = 10;
x = i + f;
...
// TODO: Разобраться с данным решением подробнее
const twoSum = function (numbers, target){
  const obj = {};
  for (let p = 0; p < numbers.length; p++){
    console.log("object =>",obj," p == ",p);
    const obj2 = obj[numbers[p]];
    console.log(obj2," = > obj 2");
     if (obj2 >= 0){
       return [obj2, p];
     } else {
       const numToFind = target - numbers[p];
       console.log(numToFind,"   2");
       obj[numToFind] = p;
       console.log(obj[numToFind],"   3");
     }
   }
  return null;
};
...
function rgb(r,g,b){
  ...
  for (let i = 0; i < 3; i ++){
    ...
    res += Math.floor(Math.abs(str[i]/16)).toString(16);
    res += Math.round(Math.abs(str[i]%16)).toString(16);
  }
  return res.toUpperCase();   
  }
console.log(rgb(0,5,6));
// end of RGBtoHEX kata, 5 kyu